...
function flattenParentOptions(nodes: ChartAccountNode[], depth = 0, result: ParentOption[] = []): ParentOption[] {
  nodes.forEach((node) => {
    result.push({
      code: node.code,
      depth,
    ...
    if (node.children?.length) {
      flattenParentOptions(node.children, depth + 1, result);
    }
  });
...
export function NewAccountModal({ accounts, onClose, onSave }: NewAccountModalProps) {
  ...
  const parentOptions = useMemo(() => flattenParentOptions(accounts), [accounts]);
  ...
  const handleSave = () => {
    ...
    if (parentOptions.some((option) => option.code === code)) {
    ...
    <div className="journal-modal__overlay" onClick={onClose}>
      ...
      >
        <div className="journal-modal__header">
          <h2 id="new-account-modal-title">New Account</h2>
        ...
        >
          ...
          <div className="journal-modal__grid journal-modal__grid--two">
            ...
            <div className="journal-modal__field" ref={parentRef}>
              ...
              {parentMenuOpen ? (
                <div className="journal-modal__selectMenu" role="listbox" aria-label="Parent account" style={{ maxHeight: 260, overflowY: "auto" }}>
                  <button
                    type="button"
